...
class CoNAL(nn.Module):
    def __identity_init(self, shape):
        ...
        if len(shape) == 3:
            ...
            for i in range(shape[1]):
                out[i, i] = 2
        return torch.Tensor(out).to("cuda"if torch.cuda.is_available() else 'cpu')
        # return torch.Tensor(out).to("cpu")
    ...
    def __init__(self, num_annotators, input_dims, num_class, rate=0.5, conn_type='MW', backbone_model=None,
        ...
        if backbone_model == 'vgg16':
            self.backbone_model = VGG('VGG16').to("cuda"if torch.cuda.is_available() else 'cpu')
        ...
        if self.common_module == 'simple':  ##
            com_emb_size = 80
            self.user_feature_vec = torch.from_numpy(user_feature).float().to("cuda"if torch.cuda.is_available() else 'cpu')
            ...
            self.single_weight = nn.Linear(20, 1, bias=False) #这个层将用于计算权重

    def simple_common_module(self, input):  # 返回工人特征和任务难度的相关性-->输出一个概率值
        ...
        # 对实例难度张量进行归一化处理，确保其数值范围在0到1之间
        user_feature = self.user_feature_1(self.user_feature_vec)
        user_feature = F.normalize(user_feature)
        # 对用户特征向量进行线性变换并归一化处理，以确保数值范围在0到1之间
        common_rate = torch.einsum('ij,kj->ik', (instance_difficulty, user_feature.float()))
        # 将两个张量的乘积求和，以生成一个新的张量，表示每个实例与每个用户之间的相关性
        common_rate = torch.sigmoid(common_rate)
            # torch.nn.functional.sigmoid(common_rate)
        return common_rate

    def forward(self, input, y=None, mode='train', support=None, support_t=None, idx=None):
        # 前向传播
        crowd_out = None
        if self.backbone_model:# 使用VGG训练
            # data_reshaped=input.reshape(64,3,32,32)
            # data_normalized=data_reshaped/np.max(data_reshaped)
            cls_out = self.backbone_model(input)
        else:# backbone主干网络训练的-->和CONLA一样
        ...
        if mode == 'train':
            ...
            # 通过公共概率和个体概率的加权求和
            # 将cls_out中的第一个维度（记为i）与self.kernel中的第二个维度（记为k）进行批量矩阵乘法。
            crowd_out = common_rate[:, :, None] * common_prob[:, None, :] + (
                        1 - common_rate[:, :, None]) * indivi_prob  # single instance
...
#     model = CoNAL(num_annotators=56, num_class=8,
#                       input_dims=8192).to(device)
#     print(summary(model, (8192)))

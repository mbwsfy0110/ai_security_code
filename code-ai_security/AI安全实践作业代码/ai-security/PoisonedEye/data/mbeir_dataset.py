...
class MBEIRQueryDataset(MBEIRDatasetBase):
    def __init__(
        ...
        shuffle_cand=False,  # Whether to shuffle the candidates
        returns=None,  # Catch any return-related settings
        print_config=True,  # Whether to print the dataset config
    ):
        super().__init__(mbeir_data_dir, img_preprocess_fn)
    ...
    def __getitem__(self, index):
        data = self.query_data[index]
        text = data["query_txt"]
        image = Image.open(os.path.join(self.mbeir_data_dir, data["query_img_path"])).convert('RGB')
        if 'answer' in json.loads(data["query_src_content"]): # infoseek
            answer = json.loads(data["query_src_content"])['answer']
            answer_eval = json.loads(data["query_src_content"])['answer_eval']
            question_type = json.loads(data["query_src_content"])['question_type']
            instance = {
                "txt": text,
                "img": image,
